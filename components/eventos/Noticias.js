...
import { formatearFecha, formatearFechaString, formatearHora } from '../../utilities/formater/FormateadorModel';
    ...
    return (
        <View style={tw`px-3`}>
            {noticias.map((noticia) => {
                ...
                const icono = obtenerIconoEvento(noticia.Tipo);
                            ...
                            <View style={tw`flex-row items-start justify-between mb-3`}>
                                <View style={tw`flex-row items-center flex-1 mr-3`}>
                                    <View style={[tw`w-10 h-10 rounded-full justify-center items-center mr-3`, { backgroundColor: colorPrioridad + '20' }]}>
                                        <FontAwesome5 name={icono} size={16} color={colorPrioridad} />
                                    </View>
                                ...
                                <View style={tw`flex-row items-center justify-between`}>
                                    <View style={tw`flex-row items-center`}>
                                        ...
                                        <Text style={tw`text-xs text-gray-500 ml-2`}>
                                            Fecha: {`${formatearFecha((noticia.Fecha))} - ${formatearHora(noticia.Hora)}`}
                                        </Text>
                                    </View>
                                </View>
                            )}
